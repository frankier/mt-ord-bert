...
def init_weights(training_args, args, model_conf, model, tokenizer, dataset):
    if args.pilot_quantiles:
        ...
        if model_conf["is_ordinal"]:
            model.pilot_quantile_init(dataset["train"], tokenizer, args.pilot_sample_size, training_args.per_device_train_batch_size, peak_class_prob=args.peak_class_prob)
        else:
            model.pilot_quantile_init(dataset["train"], tokenizer, args.pilot_sample_size, training_args.per_device_train_batch_size)
    if args.pilot_train_init:
        model.pilot_train_init(dataset["train"], tokenizer, training_args.per_device_train_batch_size)
...
def modconf(name, *args, **kwargs):
    is_kwargs = {"has_latent", "is_mono", "is_regress", "is_ordinal", "has_continuous_output", "is_threshold"}
    for k in is_kwargs:
        kwargs[k] = k in args
...
MODEL_ALIAS_DECODE = dict([
    modconf("regress", "has_latent", "is_regress", "has_continuous_output", loss="mse"),
    modconf("regress_l1", "has_latent", "is_regress", "has_continuous_output", loss="mae"),
    modconf("regress_adjust_l1", "has_latent", "is_regress", "has_continuous_output", loss="adjust_l1"),
    modconf("mono", "has_latent", "is_mono", "has_continuous_output", loss="mse"),
    modconf("mono_l1", "has_latent", "is_mono", "has_continuous_output", loss="mae"),
    modconf("mono_adjust_l1", "has_latent", "is_mono", "has_continuous_output", loss="adjust_l1"),
    *(modconf(link, "has_latent", "is_ordinal", link=link) for link in link_registry),
    modconf("class"),
...
class OrdinalPredProc:
    ...
    def proc_logits(self, logits):
        label_dists = [self.link.label_dist_from_logits(li) for li in logits[0].unbind()]
        return (
            logits[1],
            label_dists,
            *summarize_label_dists(label_dists).values(),
...
class TrainerAndEvaluator:
    ...
    def __init__(self):
        ...
        self.model, self.label_names, self.pred_proc, self.proc_logits = self.get_model(self.model_conf, self.args, self.num_labels)
        self.eval_dataset = self.prepare_eval_dataset(self.args, self.dataset, self.label_names)
        self.training_args.label_names = self.label_names
        self.training_args.optim = "adamw_torch"
        self.optimizers = self.get_optimizers(self.model_conf, self.model, self.training_args, self.args)
    ...
    def get_multi_scale_model(cls, model_conf, args, num_labels, base_model):
        ...
        if model_conf["name"] == "class":
            ...
            model_kwargs = {**model_kwargs, "link": model_conf["link"], "discrimination_mode": args.discrimination_mode}
        else:
            print(f"Unknown model type {args.model}", file=sys.stderr)
            sys.exit(-1)
        with silence_warnings():
    ...
    def decode_labels(self, labels):
        if len(self.label_names) == 2:
            ...
            batch_num_labels = torch.full(len(labels), self.num_labels, dtype=torch.uint8)
    ...
    def compute_metrics(self, eval_pred):
        ...
        with self.eval_dump_ctx(step):
            res = {}
            preds = self.pred_proc.postprocess(pred_label_dists, batch_num_labels=batch_num_labels)
            if self.args.dump_results:
                self.dump_writer.add_info_full("test", **flatten_dump(preds))
            if "hidden" in preds:
                res.update(self.refit_latent(task_ids, preds["hidden"], batch_num_labels, labels))
            ...
            if "pred" in preds:
                res.update(evaluate_predictions(preds["pred"], labels, batch_num_labels, task_ids))
            if "agg_preds" in preds:
                evaluate_pred_dist_avgs(preds["agg_preds"], labels, batch_num_labels, task_ids)
                if self.model_conf["name"] != "class":
                    raise ValueError(f"Do not know how to process metrics for {self.model_conf['name']}")
    ...
    def log_eval_on_train_data_class(self):
        for batch, result in inference_run(
            ...
            self.dump_writer.add_info_chunk(
                "train",
                **flatten_dump(self.pred_proc.postprocess(nested_numpify(self.pred_proc.proc_logits(result)), batch.scale_points))
            )
    ...
    def mk_eval_buffers(self):
        ...
        for task_id in self.eval_dataset["train"]["task_ids"]:
            cnts[task_id] += 1
        regressor_buffers = {task_id: (np.empty(cnt), np.empty(cnt, dtype=np.uint8)) for task_id, cnt in cnts.items()}

        scale_points_map = {}
        for ds in self.eval_dataset.values():
            for row in ds:
                scale_points_map[row["task_ids"]] = row["scale_points"]
        return regressor_buffers, np.empty(len(self.eval_dataset["train"])), scale_points_map
    ...
    def train(self):
        ...
        self.regressor_buffers, self.train_hidden_buffer, self.scale_points_map = self.mk_eval_buffers()

        init_weights(training_args, args, self.model_conf, self.model, self.tokenizer, self.eval_dataset)
        ...
        if args.dump_initial_model is not None:
            self.trainer.save_model(training_args.output_dir + "/" + args.dump_initial_model)
